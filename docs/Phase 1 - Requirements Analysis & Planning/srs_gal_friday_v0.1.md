...
Priority levels (e.g., High, Medium, Low) are assigned. `[MVP]` tag indicates requirement is part of the initial Minimum Viable Product scope.
...
*(Note: `[MVP]` indicates requirement is part of the Minimum Viable Product scope)*

### 3.1 Feature 1: Market Data Ingestion
* **FR-101:** The system **shall** establish and maintain persistent WebSocket connections to the Kraken API. `[High]` `[MVP]`
* **FR-102:** The system **shall** subscribe to the following WebSocket feeds for configured trading pairs (XRP/USD, DOGE/USD):
    * Level 2 Order Book updates ('book' feed). `[High]` `[MVP]`
    * OHLCV data at 1-minute intervals ('ohlc-1' feed). `[High]` `[MVP]`
* **FR-103:** The system **shall** parse incoming WebSocket messages for L2 book updates (asks, bids, checksums if provided) and OHLCV data. `[High]` `[MVP]`
* **FR-104:** The system **shall** reconstruct and maintain a local, real-time representation of the L2 order book for each subscribed pair. `[High]` `[MVP]`
* **FR-105:** The system **shall** detect WebSocket disconnections or errors and implement an automated reconnection strategy with exponential backoff. `[High]` `[MVP]`
* **FR-106:** The system **shall** standardize the ingested data into a consistent internal format (e.g., Python objects or dictionaries) before further processing. `[High]` `[MVP]`
* **FR-107:** The system **shall** publish or make available the standardized, real-time L2 book state and OHLCV data to downstream modules (Feature Engine, Logger) via an internal event bus or queue. `[High]` `[MVP]`
...
### 3.2 Feature 2: Feature Engineering
* **FR-201:** The system **shall** consume standardized OHLCV data (1-min). `[High]` `[MVP]`
* **FR-202:** The system **shall** calculate a configurable set of technical indicators based on OHLCV data, including but not limited to:
    * Relative Strength Index (RSI). `[High]` `[MVP]`
    ...
    * Price Rate of Change / Momentum. `[High]` `[MVP]`
    * Volatility measures (e.g., ATR, standard deviation of returns). `[Medium]`
* **FR-203:** The system **shall** consume the local L2 order book state. `[High]` `[MVP]`
* **FR-204:** The system **shall** calculate a configurable set of order book features, including but not limited to:
    * Bid-Ask Spread (absolute and percentage). `[High]` `[MVP]`
    * Order Book Imbalance (ratio of volume within N levels or price range). `[High]` `[MVP]`
...
* **FR-206:** The system **shall** make the calculated features available in a structured format (e.g., feature vector event) for the Predictive Modeling module. `[High]` `[MVP]`
* **FR-207:** The system **shall** allow configuration of parameters for all calculated features (e.g., RSI period, Bollinger Band deviation) via a configuration file. `[High]` `[MVP]`

### 3.3 Feature 3: Predictive Modeling
* **FR-301:** The system **shall** load pre-trained machine learning models from persistent storage upon startup. `[High]` `[MVP]`
* **FR-302:** The system **shall** support loading models compatible with Scikit-learn (for RF/XGBoost) and TensorFlow/PyTorch (for LSTM). `[High]` `[MVP for Scikit-learn, Medium for TF/PyTorch]`
* **FR-303:** The system **shall** consume the latest calculated features from the Feature Engineering module. `[High]` `[MVP]`
* **FR-304:** The system **shall** preprocess features as required by the specific loaded model(s) (e.g., scaling, normalization). `[High]` `[MVP]`
* **FR-305:** The system **shall** use the loaded models to generate predictions. The primary prediction target is the probability of the price moving up or down by a configurable threshold (e.g., 0.1%) within a configurable future time window (e.g., 5 minutes). `[High]` `[MVP]`
...
* **FR-308:** The system **shall** publish the final prediction probability (or combined signal) for the Strategy module. `[High]` `[MVP]`
...
### 3.4 Feature 4: Strategy Arbitration & Signal Generation
* **FR-401:** The system **shall** consume the final prediction probability from the Predictive Modeling module. `[High]` `[MVP]`
* **FR-402:** The system **shall** apply configurable strategy rules to generate trade entry signals (BUY or SELL). A primary rule **shall** be based on the prediction probability crossing a configurable threshold (e.g., > 65% for BUY, < 35% [or >65% for DOWN] for SELL). `[High]` `[MVP]`
...
* **FR-404:** The system **shall**, upon generating an entry signal, determine preliminary Stop-Loss (SL) and Take-Profit (TP) price levels based on configurable rules (e.g., fixed percentage below/above entry, ATR multiple, price target implied by prediction). `[High]` `[MVP]`
* **FR-405:** The system **shall** generate a candidate trade order event containing: Asset (XRP/USD or DOGE/USD), Side (BUY/SELL), preliminary SL price, preliminary TP price. `[High]` `[MVP]`
* **FR-406:** The system **shall** publish the candidate trade order event to the Risk Management module for assessment. `[High]` `[MVP]`
* **FR-407:** The system **shall** define logic for trade exits based on monitoring market price relative to active SL/TP levels associated with open positions. `[High]` `[MVP]`
...
### 3.5 Feature 5: Risk Management
* **FR-501:** The system **shall** consume candidate trade order events from the Strategy module. `[High]` `[MVP]`
* **FR-502:** The system **shall** access the current portfolio state (equity, open positions, overall exposure) from the Portfolio Management module. `[High]` `[MVP]`
* **FR-503:** The system **shall** enforce the maximum portfolio drawdown limits (Total: 15%, Daily: 2%, Weekly: 5% of starting capital for that period). If a limit is breached, no new trades **shall** be initiated (HALT condition FR-905). `[High]` `[MVP]`
...
* **FR-505:** The system **shall** calculate the final position size for the candidate trade using the Fixed Fractional method: `PositionSizeInQuoteCurrency = (CurrentEquity * RiskPercentPerTrade)` and `PositionSizeInBaseCurrency = PositionSizeInQuoteCurrency / EstimatedEntryPrice`. The actual position size placed will be `PositionSizeInQuoteCurrency` divided by the distance to the stop-loss in quote currency per unit of base currency: `PositionSize = (CurrentEquity * RiskPercentPerTrade) / StopLossDistancePerUnit`. RiskPercentPerTrade **shall** be configurable (0.5% - 1.0%). `[High]` `[MVP]`
* **FR-506:** The system **shall** perform the following pre-trade checks before approving any order:
    * Check if calculated position size exceeds a maximum configurable percentage of equity per asset (e.g., 10%). `[High]` `[MVP]`
    * Check if the new position would cause total portfolio exposure (sum of all open positions' value) to exceed a configurable percentage of equity (e.g., 25%). `[High]` `[MVP]`
    * Check if sufficient free balance/margin is available on Kraken to open the position. `[High]` `[MVP]`
    * Check if the calculated position size exceeds a maximum allowable order size (sanity check, e.g., max $10,000 per order). `[High]` `[MVP]`
    * Perform a 'fat finger' check: Ensure the proposed entry price is within a reasonable percentage (e.g., +/- 5%) of the current market price. `[High]` `[MVP]`
* **FR-507:** If all checks pass, the system **shall** approve the trade, attaching the calculated position size, and generate an approved order event for the Execution Handler. `[High]` `[MVP]`
* **FR-508:** If any check fails, the system **shall** reject the trade signal and log the reason. `[High]` `[MVP]`

### 3.6 Feature 6: Order Execution
* **FR-601:** The system **shall** consume approved order events (Asset, Side, Size, Entry Target, SL, TP) from the Risk Management module. `[High]` `[MVP]`
* **FR-602:** The system **shall** interact with the Kraken REST API for placing, canceling, and querying orders. `[High]` `[MVP]`
...
* **FR-604:** For trade entry, the system **shall** primarily attempt to place Limit orders at a price slightly better than the current market (e.g., join bid for BUY, join ask for SELL) or at the price suggested by the strategy. `[High]` `[MVP]`
...
* **FR-606:** Upon successful entry fill (or partial fill), the system **shall** place associated exit orders:
    * Stop-Loss order: **Shall** use a Market order type (e.g., `stop-loss` order type on Kraken) triggered at the SL price. `[High]` `[MVP]`
    * Take-Profit order: **Shall** use a Limit order type (e.g., `take-profit` order type on Kraken) at the TP price. `[High]` `[MVP]`
...
* **FR-608:** The system **shall** monitor the status of open orders and positions to trigger SL/TP logic based on market price movements and fill events. `[High]` `[MVP]`
* **FR-609:** The system **shall** handle API errors (e.g., insufficient funds, invalid parameters, rate limits) and order rejections gracefully, log the event, and potentially trigger alerts or HALT conditions. `[High]` `[MVP]`
* **FR-610:** The system **shall** publish detailed execution report events (order submissions, cancellations, fills with price/quantity/fees, errors) for logging and portfolio updates. `[High]` `[MVP]`

### 3.7 Feature 7: Portfolio Management
* **FR-701:** The system **shall** consume execution report events from the Execution Handler. `[High]` `[MVP]`
* **FR-702:** The system **shall** maintain an accurate, real-time internal record of:
    * Current cash balance (USD). `[High]` `[MVP]`
    * Current positions for each traded asset (XRP, DOGE), including quantity and average entry price. `[High]` `[MVP]`
* **FR-703:** The system **shall** calculate and track the current equity of the portfolio (Cash Balance + Market Value of Open Positions based on latest market price). `[High]` `[MVP]`
* **FR-704:** The system **shall** calculate and track realized P&L (from closed trades) and unrealized P&L (for open positions). `[High]` `[MVP]`
* **FR-705:** The system **shall** provide the current portfolio state (equity, positions, exposure) to the Risk Management module accurately and promptly when needed for pre-trade checks. `[High]` `[MVP]`
...
### 3.8 Feature 8: Logging & Auditing
* **FR-801:** The system **shall** log all major system events: startup, shutdown, connections established/lost, configuration loaded, HALT triggers, errors encountered. `[High]` `[MVP]`
* **FR-802:** The system **shall** log the complete lifecycle of every trade attempt:
    * Signal generation (prediction value, threshold, decision). `[High]` `[MVP]`
    * Risk assessment results (checks performed, position size calculated, approval/rejection). `[High]` `[MVP]`
    * Order submission details (API request). `[High]` `[MVP]`
    * Order status updates (acknowledgements, fills, cancellations, rejections) including API responses. `[High]` `[MVP]`
    * Final realized P&L for closed trades. `[High]` `[MVP]`
* **FR-803:** The system **shall** log key features and model prediction values associated with each trade signal. `[High]` `[MVP]`
* **FR-804:** Logs **shall** be timestamped accurately (UTC recommended) with millisecond precision. `[High]` `[MVP]`
* **FR-805:** Logs **shall** be written in a structured format (e.g., JSON) to rolling log files (e.g., daily rotation, max size). `[High]` `[MVP]`
* **FR-806:** Critical data points (e.g., trade details, order fills, final P&L, daily performance summaries) **shall** be persisted to the PostgreSQL database for analysis and auditing. `[High]` `[MVP]`
...
### 3.9 Feature 9: Monitoring & HALT/Recovery
* **FR-901:** The system **shall** continuously monitor the status of its connection to Kraken APIs (WebSocket and REST availability via periodic checks). `[High]` `[MVP]`
* **FR-902:** The system **shall** monitor the freshness of incoming market data feeds. If no data is received for a configurable period (e.g., 60 seconds), it indicates a problem. `[High]` `[MVP]`
...
* **FR-904:** The system **shall** monitor portfolio equity and drawdown against the defined daily, weekly, and total limits (see FR-503). `[High]` `[MVP]`
* **FR-905:** The system **shall** implement automated HALT triggers that immediately cease the placement of *new* trade entries under the following conditions:
    * Breaching daily, weekly, or total drawdown limits. `[High]` `[MVP]`
    * Experiencing a configurable number of consecutive losing trades (e.g., 5). `[Medium]`
    * Repeated critical API errors (e.g., 3 consecutive order placement failures, authentication failures). `[High]` `[MVP]`
    * Loss of reliable market data feed (staleness detected per FR-902). `[High]` `[MVP]`
    * Detection of excessive market volatility (e.g., price change % exceeding configurable threshold in short period). `[Medium]`
    * Receiving an external HALT command (e.g., via CLI or signal). `[High]` `[MVP]`
* **FR-906:** The behavior upon HALT regarding *existing* open positions **shall** be configurable:
    * Option 1 (Default): Close all open positions immediately via Market orders. `[High]` `[MVP]`
    * Option 2: Let existing positions run with their predefined SL/TP orders active. `[Medium]`
* **FR-907:** Upon triggering a HALT condition, the system **shall** log the reason clearly and send a notification alert (initially via detailed log messages; email/SMS integration is a future enhancement). `[High]` `[MVP]`
* **FR-908:** Following a HALT, the system **shall** require manual intervention (e.g., review by Administrator/User, explicit restart command via CLI/signal) before resuming the placement of new trades. No automated recovery for MVP. `[High]` `[MVP]`
...
### 4.1 User Interfaces
* **NFR-201:** The primary user interface for the Administrator/Monitor **shall** be via configuration files (e.g., YAML or JSON) for setting parameters and a command-line interface (CLI) for starting, stopping, checking status, and initiating HALT/Resume commands. `[High]` `[MVP]`
* **NFR-202:** System output for the user **shall** primarily be through structured log files and database entries. `[High]` `[MVP]`
* **NFR-203:** No Graphical User Interface (GUI) is required for the MVP. `[High]` `[MVP]`

### 4.2 Hardware Interfaces
* **NFR-204:** The system interfaces with standard cloud VM hardware (CPU, RAM, Disk, Network Interface Card). No specialized hardware is required. `[High]` `[MVP]`
...
* **NFR-301:** The system **shall** interface with the **Kraken REST API** for:
    ...
    * Fetching Historical Data (for backtesting/retraining) `[High]` `[MVP]`
* **NFR-302:** The system **shall** interface with the **Kraken WebSocket API** for:
    ...
    * (Optional/Should) Receiving real-time private order/fill updates. `[High]` `[MVP]`
* **NFR-303:** The system **shall** interface with a **PostgreSQL database server** via standard SQL protocols (e.g., using psycopg2 or SQLAlchemy) for storing relational data (trades, orders, logs). `[High]` `[MVP]`
...
* **NFR-305:** The system **shall** operate on a **Linux Operating System**, interacting via standard POSIX system calls and interfaces. `[High]` `[MVP]`
* **NFR-306:** The system **shall** utilize the **Python 3 interpreter** and its standard libraries, plus specified third-party libraries. `[High]` `[MVP]`

### 4.4 Communications Interfaces
* **NFR-401:** All communication with the Kraken REST API **shall** use HTTPS. `[High]` `[MVP]`
* **NFR-402:** All communication with the Kraken WebSocket API **shall** use Secure WebSockets (WSS). `[High]` `[MVP]`
* **NFR-403:** Communication with PostgreSQL and InfluxDB databases **should** occur over a secure network (e.g., within a VPC or using TLS if databases are exposed). `[High]` `[MVP]`
* **NFR-404:** The system requires outbound internet access on ports 443 (HTTPS) and potentially others specified by Kraken for WSS. `[High]` `[MVP]`
...
* **NFR-503:** **Throughput:** The system **shall** be capable of processing the expected volume of L2 updates and OHLCV data from Kraken for two active trading pairs without falling behind or consuming excessive resources. `[High]` `[MVP]`
* **NFR-504:** **Resource Usage:** The system **should** operate within reasonable CPU (<75% average utilization), RAM (<4GB recommended allocation initially), and Network I/O limits of the chosen cloud VM instance under normal operating conditions. `[Medium]` `[MVP]`
...
### 5.2 Safety Requirements
* **NFR-601:** The HALT mechanisms defined in FR-905 **must** function reliably to prevent excessive losses or trading under adverse conditions. `[High]` `[MVP]`
* **NFR-602:** Risk management checks (FR-506) **must** be performed before every trade execution attempt. `[High]` `[MVP]`
* **NFR-603:** In case of unhandled exceptions or critical component failures, the system **should** default to a safe state (e.g., cease new trading, log the error, potentially trigger HALT). `[High]` `[MVP]`

### 5.3 Security Requirements
* **NFR-701:** Kraken API keys **must** be stored securely and not exposed in source code or logs. Use environment variables, secure configuration files, or a secrets management service. `[High]` `[MVP]`
* **NFR-702:** API keys used for the bot **should** have permissions restricted to trading and querying balances/orders only; withdrawal permissions **must** be disabled. `[High]` `[MVP]`
* **NFR-703:** Access to the host machine, databases, and logs **should** be restricted using standard security practices (firewalls, strong passwords, SSH keys, limited user privileges). `[High]` `[MVP]`
* **NFR-704:** Dependencies (Python libraries, OS packages) **should** be kept up-to-date to patch known vulnerabilities. `[Medium]`

### 5.4 Software Quality Attributes
* **NFR-801:** **Reliability:** The system **shall** operate continuously during configured trading hours with minimal downtime. Automated reconnection (FR-105) and robust error handling (FR-609) are essential. Target uptime > 99.5% during active trading periods. `[High]` `[MVP]`
* **NFR-802:** **Maintainability:** The code **shall** be well-commented, follow consistent coding standards (e.g., PEP 8), and be organized into logical modules to facilitate understanding and future modifications. `[High]` `[MVP]`
* **NFR-803:** **Testability:** The system design **shall** support unit testing of individual modules and integration testing of workflows. Backtesting (Feature 10) serves as a key system-level test. `[High]` `[MVP]`
* **NFR-804:** **Configurability:** Key parameters (risk settings, strategy thresholds, API endpoints, feature parameters, file paths) **shall** be externalized into configuration files, not hardcoded. `[High]` `[MVP]`
...
### 5.5 Business Rules
* **NFR-901:** Trading **shall** only occur on the specified pairs: XRP/USD, DOGE/USD. `[High]` `[MVP]`
* **NFR-902:** Trading **shall** only occur on the Kraken exchange. `[High]` `[MVP]`
* **NFR-903:** All risk parameters (drawdown limits, risk per trade %) **must** be adhered to as defined (FR-503, FR-505). `[High]` `[MVP]`
* **NFR-904:** The system **must** account for Kraken's trading fees when calculating P&L and potentially in decision-making (e.g., ensuring profit targets exceed fees). `[High]` `[MVP]`
