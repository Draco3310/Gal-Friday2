...
class MarketHours:
    ...
    # Extended hours
    pre_market_open: str | None = None  # "04:00"
    after_market_close: str | None = None  # "20:00"
...
class KrakenMarketDataProvider(MarketDataProvider):
    ...
    async def _fetch_kraken_price(self, symbol: str) -> PriceData | None:
        ...
        try:
            ...
            price, bid, ask, volume = mock_prices.get(symbol, (1.0, 0.999, 1.001, 100000.0))
    ...
    async def _update_price_history(self, symbol: str, price: float) -> None:
        ...
        history = self._price_history[symbol]
        history.append((price, datetime.now(UTC)))
...
class ExchangeCalendarManager:
    ...
    async def is_trading_day(self, exchange: str, date: datetime | None = None) -> bool:
        ...
        return date_str not in market_hours.holidays

    async def get_next_market_open(self, exchange: str) -> datetime | None:
    ...
    async def get_current_price(self, symbol: str) -> float | None:
        ...
        try:
            ...
            if price_data:
                ...
                age = (datetime.now(UTC) - price_data.timestamp).total_seconds()
                ...
                self._last_successful_fetch[symbol] = datetime.now(UTC)
                self._connection_failures = 0
...
class KrakenErrorClassifier:
    ...
    def classify_error(self, error_message: str, context: dict[str, Any] | None = None) -> ErrorInstance:
    ...
    def calculate_retry_delay(self, strategy: dict[str, Any]) -> float:
        ...
        if strategy_type == RetryStrategy.IMMEDIATE.value:
            return 0.0
        if strategy_type == RetryStrategy.LINEAR_BACKOFF.value:
            return float(base_delay * (retry_count + 1))
        if strategy_type == RetryStrategy.EXPONENTIAL_BACKOFF.value:
            return float(base_delay * (2 ** retry_count))
        if strategy_type == RetryStrategy.FIXED_INTERVAL.value:
