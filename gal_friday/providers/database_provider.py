...
class DatabaseDataProvider(HistoricalDataProvider):
    ...
    def __init__(self, config: dict[str, Any], logger: logging.Logger) -> None:
        ...
        self._connection_pool: DatabaseConnectionPool | None = None
        self._ts_db: TimeSeriesDB | None = None
    ...
    async def _fetch_from_postgresql(self, request: DataRequest) -> list[HistoricalDataPoint]:
        ...
        async with self._get_db_session() as session:
            ...
            return self._aggregate_orders_to_ohlcv(list(orders), request)
    ...
    async def get_diagnostics(self) -> dict[str, Any]:
        ...
        if self._connection_pool and self._initialized:
            try:
                ...
                cast("dict[str, str]", diagnostics["connections"])["postgresql"] = f"error: {e!s}"
        ...
        if self._ts_db and self._initialized:
            try:
                ...
                cast("dict[str, str]", diagnostics["connections"])["influxdb"] = f"error: {e!s}"
                diagnostics["status"] = "degraded"
