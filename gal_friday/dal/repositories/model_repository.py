...
class ModelRepository(BaseRepository[ModelVersion]):
    ...
    async def add_model_version(self, model_version_data: dict[str, Any]) -> ModelVersion:
        ...
        'model_id' should be a UUID object if provided, or will be generated if not present
        and model expects it.
        """
        if "model_id" not in model_version_data:  # Ensure model_id is present
            model_version_data["model_id"] = model_version_data.get("model_id", uuid.uuid4())
        # Ensure datetime objects are timezone-aware
        if ("created_at" in model_version_data and
                isinstance(model_version_data["created_at"], datetime) and
                model_version_data["created_at"].tzinfo is None):
            model_version_data["created_at"] = model_version_data["created_at"].replace(tzinfo=UTC)
        if ("training_completed_at" in model_version_data and
                isinstance(model_version_data["training_completed_at"], datetime) and
                model_version_data["training_completed_at"].tzinfo is None):
            model_version_data["training_completed_at"] = model_version_data[
    ...
    async def update_model_version_stage(
        ...
        model_id: uuid.UUID,
        new_stage: str,
        deployed_by: str | None = None,  # Assuming stage is string
    ) -> ModelVersion | None:
        """Update model version's stage.

        If promoting to 'production', creates a deployment record.
        """
    ...
    async def get_active_deployment(self, model_name: str) -> ModelDeployment | None:
        ...
        async with self.session_maker() as session:
            stmt = (
                ...
                .where(ModelVersion.model_name == model_name, ModelDeployment.is_active) # E712
    ...
    async def get_deployments_for_model_version(
        ...
        # could also be BaseRepository[ModelDeployment].find_all
        async with self.session_maker() as session:
