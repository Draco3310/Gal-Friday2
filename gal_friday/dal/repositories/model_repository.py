...
class ModelRepository(BaseRepository[ModelVersion]):
    ...
    async def add_model_version(self, model_version_data: dict[str, Any]) -> ModelVersion:
        ...
        if (
            "created_at" in model_version_data
            and isinstance(model_version_data["created_at"], datetime)
            and model_version_data["created_at"].tzinfo is None
        ):
            model_version_data["created_at"] = model_version_data["created_at"].replace(tzinfo=UTC)
        if (
            "training_completed_at" in model_version_data
            and isinstance(
                model_version_data["training_completed_at"],
                datetime,
            )
            and model_version_data["training_completed_at"].tzinfo is None
        ):
            model_version_data["training_completed_at"] = model_version_data["training_completed_at"].replace(
                tzinfo=UTC,
            )
    ...
    async def get_model_versions_by_name(self, model_name: str, version: str | None = None) -> Sequence[ModelVersion]:
    ...
    async def get_latest_model_version_by_name(self, model_name: str) -> ModelVersion | None:
        ...
        versions = await self.find_all(filters={"model_name": model_name}, order_by="created_at DESC", limit=1)
        return versions[0] if versions else None

    async def get_model_versions_by_stage(
        self,
        model_name: str,
        stage: str,  # Assuming stage is a string now
    ) -> Sequence[ModelVersion]:
        """Get model versions by name and stage."""
        return await self.find_all(filters={"model_name": model_name, "stage": stage}, order_by="created_at DESC")
    ...
    async def update_model_version_stage(
        ...
        try:
            ...
            from gal_friday.model_lifecycle.enums import (  # noqa: PLC0415 - architectural: avoids circular import between model repository and lifecycle enums
                ModelStage,
            )

            target_stage = ModelStage(new_stage.lower())
            current_stage = await self._get_current_stage(model_id)
            validation_metadata = self._prepare_validation_metadata(metadata, approval_id, deployed_by)

            # Validate and perform transition
            await self._validate_stage_transition(
                model_id,
                current_stage,
                target_stage,
                validation_metadata,
            ...
            return await self._update_model_stage(
                model_id,
                new_stage,
                current_stage,
                target_stage,
            ...
            self.logger.exception(f"Failed to update model version stage for {model_id}")
            raise

    async def _get_current_stage(self, model_id: uuid.UUID) -> ModelStage | None:
        ...
        if current_model and current_model.stage:
            try:
                ...
                return ModelStage(current_model.stage.lower())
            except ValueError:
                self.logger.warning(
    ...
    def _prepare_validation_metadata(
        self,
    ...
    async def _validate_stage_transition(
        ...
        """Validate stage transition and handle validation failure."""
    ...
    async def _handle_validation_failure(
        ...
        """Handle stage transition validation failure."""
    ...
    async def _update_model_stage(
        ...
        """Update model stage and handle post-update tasks."""
        updated_model = await self.update(model_id, {"stage": new_stage})

        if updated_model:
            # Record successful transition
            self._record_successful_transition(
                model_id,
                current_stage,
                target_stage,
                deployed_by,
            ...
            # Handle production deployment if needed
    ...
    def _record_successful_transition(
        ...
        """Record successful stage transition in audit trail."""
        self.stage_manager.record_stage_transition(
            model_id=str(model_id),
            from_stage=current_stage,
            to_stage=target_stage,
    ...
    async def _create_deployment_record(
        ...
        async with self.session_maker() as session:
            ...
            self.logger.info(
                f"Created new deployment record {new_deployment.deployment_id} for model_id {model_version.model_id}",
                source_module=self._source_module,
            )
    ...
    async def get_active_deployment(self, model_name: str) -> ModelDeployment | None:
        ...
        async with self.session_maker() as session:
            stmt = (
                ...
                .where(ModelVersion.model_name == model_name, ModelDeployment.is_active.is_(True))
    ...
    async def promote_model_to_production(
        ...
        if performance_metrics:
            ...
            metrics_as_any: dict[str, Any] = dict(performance_metrics.items())
