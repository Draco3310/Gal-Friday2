...
class StrategyPerformanceAnalyzer:
    ...
    async def analyze_strategy_performance(
        ...
        if strategy_id in self._metrics_cache:
            ...
            if datetime.now(UTC) - cache_time < self._cache_ttl:
        ...
        try:
            ...
            self._metrics_cache[strategy_id] = (datetime.now(UTC), metrics)
            ...
            if hasattr(self, "performance_metrics_repository") and self.performance_metrics_repository:
                try:
                    ...
                    self.logger.exception(
                        "Failed to persist performance metrics: ",
                        source_module=self._source_module,
                    )
    ...
    async def _fetch_strategy_trades(
        ...
        start_date = datetime.now(UTC) - lookback_period
        ...
        try:
            ...
            self.logger.exception(
                f"Failed to fetch trades for {strategy_id}: ",
                source_module=self._source_module)
            return []
    ...
    async def _fetch_portfolio_snapshots(
        ...
        try:
            ...
            self.logger.exception(
                "Failed to fetch portfolio snapshots: ",
                source_module=self._source_module)
            return []
    ...
    async def _fetch_execution_metrics(
        ...
        try:
            ...
            self.logger.exception(
                f"Failed to fetch execution metrics for {strategy_id}: ",
                source_module=self._source_module)
            return []
...
class MarketConditionMonitor:
    ...
    def __init__(self, logger: LoggerService, config: dict[str, Any]) -> None:
        ...
        self._condition_cache: tuple[datetime, MarketConditionSnapshot] | None = None
    ...
    async def assess_market_conditions(
        ...
        try:
            ...
            # Update cache
            self._condition_cache = (datetime.now(UTC), snapshot)
    ...
    def _classify_volatility_regime(self, volatility: float) -> MarketRegime:
        ...
        if volatility > self._volatility_thresholds["high"]:
    ...
    def _classify_liquidity_condition(self, spread_bps: float) -> LiquidityCondition:
        ...
        if spread_bps > self._liquidity_thresholds["reduced_spread_bps"]:
    ...
    def _determine_active_sessions(self) -> list[str]:
        ...
        if current_hour_utc >= 23 or current_hour_utc < 9:
...
class StrategySelector:
    ...
    def _calculate_performance_score(
        ...
        score = 0.0
        for data in performance_factors.values():
    ...
    def _calculate_market_fit_score(
        ...
        if market_conditions.trend_state in [TrendState.STRONG_UPTREND, TrendState.STRONG_DOWNTREND]:
            ...
            trend_score = 0.9 if "trend" in metrics.strategy_id.lower() else 0.6
        # Sideways markets favor mean reversion
        elif "reversion" in metrics.strategy_id.lower() or "scalp" in metrics.strategy_id.lower():
            trend_score = 0.9
        else:
            trend_score = 0.7
    ...
    def _generate_recommendation(
        ...
        if not meets_thresholds:
            return "reject"
        if not risk_evaluation["passes_checks"]:
            return "monitor"
        if composite_score >= 0.75 and improvement >= 0.15:
            return "deploy"
    ...
    def _calculate_decision_confidence(
        ...
        stability_confidence = 1.0 - min(abs(performance_trend) * 10, 1.0) if performance_trend is not None else 0.5
...
class StrategyOrchestrator:
    ...
    def __init__(
        ...
        self._active_transition: StrategyTransitionPlan | None = None
    ...
    async def _execute_transition_phase(
        ...
        while datetime.now(UTC) < phase_end:
    ...
    async def _complete_transition(
        ...
        transition_plan.completed_at = datetime.now(UTC)
    ...
    def get_active_transition(self) -> StrategyTransitionPlan | None:
...
class StrategySelectionSystem:
    ...
    def __init__(
        ...
        # Selection state
        self._last_selection_time: datetime | None = None
    ...
    def _should_run_selection(self) -> bool:
        ...
        if self._last_selection_time:
            time_since_last = datetime.now(UTC) - self._last_selection_time
            return time_since_last >= self._selection_frequency
        return True  # First run

    async def run_strategy_selection(self) -> StrategyEvaluationResult | None:
        ...
        try:
            ...
            # Update timestamp
            self._last_selection_time = datetime.now(UTC)
    ...
    async def _create_selection_context(self) -> StrategySelectionContext:
        ...
        try:
            ...
            self.logger.exception(
                "Failed to retrieve portfolio state: ",
                source_module=self._source_module)
            portfolio_state = {}
        ...
        try:
            ...
            self.logger.exception(
                "Failed to retrieve risk budget: ",
                source_module=self._source_module)
            risk_budget = Decimal(0)
    ...
    async def force_strategy_evaluation(self) -> StrategyEvaluationResult | None:
