...
class BacktestingEngine:
    ...
    async def _initialize_services(self) -> None:
        ...
        This method sets up:
        - `LoggerService`: For consistent logging across components.
        - `PubSubManager`: A local pub/sub system for event communication within the backtest.
        - `FeatureEngine`: The refactored feature engine, which will load feature definitions
          from the registry, activate/override based on app config, and generate features.
          It's configured to use the local `PubSubManager`.

        It then starts the `FeatureEngine` and subscribes a backtesting-specific handler
        (`_backtest_feature_event_handler`) to capture `FEATURES_CALCULATED` events,
        primarily for logging or direct inspection within the `BacktestingEngine`.
        Other services (like `PredictionService` and `StrategyArbitrator`), when started,
        are expected to subscribe to relevant events (e.g., `FEATURES_CALCULATED`,
        `PREDICTION_GENERATED`) on this same `PubSubManager` instance to receive data.
        """
        self.logger.info("Initializing backtesting services (PubSub, FeatureEngine)...")

        # Instantiate LoggerService if not already available/passed differently
        # For this integration, we'll assume BacktestingEngine's self.logger can be used
        # or a simple LoggerService is created if FeatureEngine/PubSub strictly need the class.
        # If self.config is a ConfigManager instance:
        log_config = self.config.get("logging", {}) if hasattr(self.config, 'get') else {}
        if not isinstance(log_config, dict): log_config = {}
        self.logger_service = LoggerService(config=log_config, is_backtest=True) # Basic instantiation

        self.pubsub_manager = PubSubManager(logger_service=self.logger_service)

        app_config_all = self.config.get_all() if hasattr(self.config, 'get_all') else self.config
        if not isinstance(app_config_all, dict):
            self.logger.error("Full application config is not a dictionary. FeatureEngine might not initialize correctly.")
            app_config_all = {} # Provide empty dict to prevent crash, FE will log warnings.
    ...
    async def _backtest_feature_event_handler(self, event_dict: Dict[str, Any]) -> None:
        """Handles feature events during backtesting to capture features for strategies."""
    ...
    async def _execute_simulation(  # noqa: PLR0912
        ...
        Args:
            services: Dictionary of initialized services. These services (e.g.,
                      `PortfolioManager`, `ExecutionHandler`, `PredictionService`,
                      `StrategyArbitrator`) should be instantiated with the same
                      `PubSubManager` and `LoggerService` created in `_initialize_services`
                      to ensure they participate in the backtesting event flow.
            run_config: Configuration for the backtest run.
        """
