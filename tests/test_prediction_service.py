        ...
        "prediction": {
            "models": {
                "price_direction": {
                    ...
                    "features": ["rsi_14", "macd", "bbands_width", "volume_change", "price_momentum"],
                ...
                "volatility": {
                    ...
                    "features": ["historical_volatility", "volume", "returns"],
...
def test_prediction_service_feature_calculation(mock_feature_engine, mock_load_model, 
                                              prediction_config, event_bus, mock_ohlcv_data):
    ...
    mock_model.predict.return_value = np.array([1, 0, 1, 1, 0])  # Sample predictions
    ...
    assert set(features.columns) >= set(prediction_config["prediction"]["models"]["price_direction"]["features"])


@patch("gal_friday.prediction_service.load_model")
def test_prediction_service_make_predictions(mock_load_model, prediction_config, event_bus):
    ...
    price_model.predict_proba.return_value = np.array([[0.3, 0.7]])  # 70% confidence
    
    # Mock the volatility model
    vol_model = MagicMock()
    vol_model.predict.return_value = np.array([0.15])  # 15% predicted volatility
    ...
    price_events = [e for e in published_events if e.prediction_type == "price_direction"]
    ...
    vol_events = [e for e in published_events if e.prediction_type == "volatility"]
...
@patch("gal_friday.prediction_service.load_model")
def test_prediction_service_handle_market_data(mock_load_model, prediction_config, event_bus):
...
def test_prediction_service_retrain_models(mock_save_model, mock_load_model, 
                                         prediction_config, event_bus, mock_ohlcv_data):
