...
def test_risk_manager_order_validation(risk_config, event_bus):
    ...
    # Create an order that would exceed max position size when combined with existing position
    position_limit_order = OrderEvent(
        timestamp=datetime.now(),
        symbol="BTC/USD",
        order_type="MARKET",
        quantity=0.06,  # Would make BTC position 11% (5% + 6%), above 10% limit
        direction="BUY"
    )
...
def test_risk_manager_drawdown_protection(risk_config, event_bus):
    ...
    mock_portfolio_manager.get_current_drawdown.return_value = 0.20  # 20% drawdown, exceeds 15% limit
