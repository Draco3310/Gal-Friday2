...
class MockPredictionService:
    ...
    async def stop(self):
        self._is_running = False
        # In a real scenario, ensure unsubscribe happens cleanly
        # await self.pubsub.unsubscribe(EventType.FEATURES_CALCULATED, self.handle_feature_event)
    ...
    async def handle_feature_event(self, event_dict: dict): # PubSub delivers dicts
        ...
        self.logger.info(f"{self.source_module} published PredictionEvent for {trading_pair} at {timestamp_prediction_for}")
...
class TestBacktestingEngineFeatureFlow(unittest.IsolatedAsyncioTestCase):
    ...
    def setUpClass(cls):
        ...
        with open(cls.feature_registry_path, "w") as f:
        ...
        with open(cls.historical_data_path, "w") as f:
    ...
    def tearDownClass(cls):
        ...
        if cls.feature_registry_path.exists():
            os.remove(cls.feature_registry_path)
        if cls.historical_data_path.exists():
            os.remove(cls.historical_data_path)
        # Could remove TEST_TEMP_DIR if empty and desired

    async def test_features_flow_to_strategy_arbitrator(self):
        ...
        # This implies a potential instantiation order issue or need for deferred setup.
        # For the test, let's try to instantiate them in an order that satisfies dependencies.
        # LoggerService(config_manager, pubsub_manager)
        # PubSubManager(logger_service)
        ...
        # exchange_info_svc_config = config_manager.get("exchange_info_service", {}) # Config can remain
        # exchange_info_service = ExchangeInfoService(config=exchange_info_svc_config) # Cannot instantiate ABC

        # SimulatedMarketPriceService needs historical data provider, BacktestingEngine creates one internally
        # but StrategyArbitrator needs a MarketPriceService instance.
        # We'll create a simple one for StrategyArbitrator.
        ...
        with patch("gal_friday.strategy_arbitrator.StrategyArbitrator._validate_confirmation_rule",
            ...
            for call_args in mock_validate_rule.call_args_list:
                ...
                # features_arg = args[2] if len(args) > 2 else kwargs.get('features')
                ...
                # An alternative: patch the *caller* of `_validate_confirmation_rule` which is `_apply_secondary_confirmation`
                # or make `_validate_confirmation_rule` store its received features.

                # Simpler: Check what the mock_prediction_service published, as that's what SA should get.
                # This is already implicitly tested by mock_validate_rule being called,
                # if the rules use features that were indeed published.
                ...
                # Example: strategy_arbitrator_instance._validate_confirmation_rule(rule_dict, features_dict, "pair", "BUY")
                # call_args.args = (rule_dict, features_dict, "pair", "BUY")
