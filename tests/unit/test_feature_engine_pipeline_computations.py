...
def sample_l2_books_series(sample_l2_book_valid, sample_l2_book_empty) -> pd.Series:
    ...
    idx = pd.to_datetime([
        datetime.now(timezone.utc) - pd.Timedelta(seconds=2),
...
def sample_trades_deque() -> deque:
    return deque([
        {'timestamp': datetime(2023, 1, 1, 12, 0, 0, tzinfo=timezone.utc), 'price': Decimal('100.0'), 'volume': Decimal('1.0'), 'side': 'buy'},
        {'timestamp': datetime(2023, 1, 1, 12, 0, 10, tzinfo=timezone.utc), 'price': Decimal('100.1'), 'volume': Decimal('0.5'), 'side': 'sell'},
        {'timestamp': datetime(2023, 1, 1, 12, 0, 20, tzinfo=timezone.utc), 'price': Decimal('100.2'), 'volume': Decimal('1.2'), 'side': 'buy'},
        {'timestamp': datetime(2023, 1, 1, 12, 1, 0, tzinfo=timezone.utc), 'price': Decimal('100.3'), 'volume': Decimal('0.8'), 'side': 'buy'}, # Next bar
        {'timestamp': datetime(2023, 1, 1, 12, 1, 15, tzinfo=timezone.utc), 'price': Decimal('100.2'), 'volume': Decimal('0.3'), 'side': 'sell'},
    ])
...
def sample_bar_start_times() -> pd.Series:
    return pd.Series([
        ...
        datetime(2023, 1, 1, 12, 2, 0, tzinfo=timezone.utc) # Bar with no trades
    ])
