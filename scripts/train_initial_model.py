...
def load_historical_data(config: ConfigManager) -> pd.DataFrame:
    ...
    try:
        ...
        log.info(f"{len(df)} rows remaining after initial NaN drop.")
...
def generate_features(df: pd.DataFrame, config: ConfigManager) -> pd.DataFrame:
    ...
    df.columns = [col.lower() for col in df.columns]  # Example: lowercase all
...
def split_data(df: pd.DataFrame, config: ConfigManager) -> tuple:
...
def save_model(model: xgb.XGBClassifier, config: ConfigManager):
