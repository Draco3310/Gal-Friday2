...
class DependencyMissingError(SetupError):
    ...
    def __init__(self, component: str, dependency: str, message: Optional[str] = None):
        self.component = component
        self.dependency = dependency
        if message is None:
            message = f"Dependency '{dependency}' is missing or unavailable for component '{component}'."
        super().__init__(message)

class ComponentInitializationError(SetupError):
    """Errors during the initialization of a critical component."""

    def __init__(self, component_name: str, details: Optional[str] = None, message: Optional[str] = None):
...
class UnsupportedModeError(OperationalError, ValueError):
    ...
    def __init__(self, mode: str, supported_modes: Optional[list[str]] = None, message: Optional[str] = None):
        self.mode = mode
        self.supported_modes = supported_modes
        if message is None:
            message = f"Unsupported operational mode: '{mode}'."
            if supported_modes:
                message += f" Supported modes are: {', '.join(supported_modes)}."
            else:
                message += " Please ensure the mode is valid."
        super().__init__(message)

# Example of a more specific initialization error if needed later
...
class InvalidLoggerTableNameError(ConfigurationError):
    ...
    def __init__(self, table_name: str, allowed_tables: set[str]):
...
class UnsupportedParamsTypeError(TypeError):
    ...
    def __init__(self, params_type: type):
...
class ServiceInstantiationFailedExit(CriticalExit):
    ...
    def __init__(self, original_exception: Optional[Exception] = None) -> None:
...
class ExecutionHandlerInstantiationFailedExit(CriticalExit):
    ...
    def __init__(self, mode: str, original_exception: Optional[Exception] = None) -> None:
        message = f"Execution Handler failed to instantiate for mode: '{mode}'. Application exiting."
        super().__init__(message)
...
class RiskManagerInstantiationFailedExit(CriticalExit):
    ...
    def __init__(self, component_name: str = "RiskManager", original_exception: Optional[Exception] = None) -> None:
...
class MarketPriceServiceUnsupportedModeError(UnsupportedModeError):
    ...
    def __init__(self, mode: str, supported_modes: Optional[list[str]] = None, message: Optional[str] = None) -> None:
        if message is None:
            message = f"Cannot instantiate MarketPriceService for mode '{mode}'. Missing or incompatible implementation."
...
class MarketPriceServiceCriticalFailureExit(CriticalExit):
    ...
    def __init__(self, original_exception: Optional[Exception] = None) -> None:
        super().__init__("MarketPriceService instantiation failed critically. Application exiting.")
...
class PortfolioManagerInstantiationFailedExit(CriticalExit):
    ...
    def __init__(self, original_exception: Optional[Exception] = None) -> None:
        super().__init__("PortfolioManager instantiation failed. Application exiting.")
...
class LoggerServiceInstantiationFailedExit(CriticalExit):
    ...
    def __init__(self, original_exception: Optional[Exception] = None) -> None:
        super().__init__("LoggerService instantiation failed. Application exiting.")
...
class PubSubManagerInstantiationFailedExit(CriticalExit):
    ...
    def __init__(self, original_exception: Optional[Exception] = None) -> None:
        super().__init__("PubSubManager core instantiation failed. Application exiting.")
...
class ConfigurationLoadingFailedExit(CriticalExit):
    ...
    def __init__(self, original_exception: Optional[Exception] = None) -> None:
        super().__init__("Configuration loading failed. Application exiting.")
